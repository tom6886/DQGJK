...
namespace DQGJK.Web.Controllers
{
    public class LogInfoController : BaseController
    {
        ...
        public LogInfoController(DBContext context)
        {
            _context = context;
        }
        ...
        public PartialViewResult List(string province, string city, string country, string stationCode, string startDate, string endDate, int pi = 1)
        {
            ...
            var query = _context.LogInfo.AsQueryable();
            ...
            if (!string.IsNullOrEmpty(endDate)) { query = query.Where(q => q.CreateTime < Convert.ToDateTime(endDate).AddDays(1)); }

            Pager pager = new Pager(query.Count(), pi);

            List<LogInfo> list = query.OrderByDescending(q => q.CreateTime).Skip((pager.PageIndex - 1) * 10).Take(10).ToList();
